...
def learning_paths_list():
    paths = LearningPaths.query.filter_by(published=True).order_by(
        LearningPaths.track.asc(), LearningPaths.title.asc()
    ).all()
    ...
    return render_template(
        ...
        paths=paths, progress_by_path=progress_by_path,
    )
...
def learning_path_detail(slug):
    ...
    return render_template(
        ...
        path=path, steps=steps, done=done, total=total, pct=pct,
    )
...
def progress_dashboard():
    ...
    for path in paths:
        ...
        total_done_all += done
        rows.append({
            "path": path, "done": done, "total": total,
            "pct": round((done / total) * 100) if total else 0,
        })

    overall_pct = round((total_done_all / total_steps_all) * 100) if total_steps_all else 0

    return render_template(
        "platform_plus/progress_dashboard.html",
        rows=rows, overall_pct=overall_pct,
        total_done_all=total_done_all, total_steps_all=total_steps_all,
    )
...
def admin_learning_path_edit(path_id):
    ...
    all_challenges = Challenges.query.order_by(Challenges.category, Challenges.name).all()
    used_ids = {s.challenge_id for s in path.steps}
    available = [c for c in all_challenges if c.id not in used_ids]
    return render_template(
        "platform_plus/admin_learning_path_edit.html", path=path, available=available,
    )


@learning_paths_bp.route("/admin/learning-paths/<int:path_id>/add-step", methods=["POST"])
@admins_only
def admin_learning_path_add_step(path_id):
    ...
    step = LearningPathSteps(
        path_id=path.id, challenge_id=challenge_id,
        position=next_position, note=request.form.get("note", "").strip(),
    )
    db.session.add(step)
    db.session.commit()
...
@learning_paths_bp.route("/admin/learning-paths/steps/<int:step_id>/remove", methods=["POST"])
@admins_only
